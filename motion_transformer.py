...
class MotionTransformer(nn.Module):
    ...
    def _mask(src_size, trg_size, device):
        mask = torch.triu(
            torch.ones(src_size + trg_size, src_size + trg_size, device=device),
            diagonal=1
        )
        mask[:, :src_size] = 0
        return mask.bool()
    ...
    @torch.no_grad()
    def inference(self, src, trg, num_steps=1, src_key_padding_mask=None):
        preds = []
        for _ in range(num_steps):
            trg_ = torch.cat([trg[:, 0:1, :]] + preds, dim=1)
            out = self.forward(src, trg_, src_key_padding_mask=src_key_padding_mask)
            preds.append(out[:, -1:, :])
    ...
    def load_weight(self, path, map_location=None):
